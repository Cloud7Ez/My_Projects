...
select year_id,to_char(round(sum(sales), 0), '$999,999,999') revenue 
	from sales
		group by 1
			order by 2 desc;
			
--Calculating revenue by month
select month_id,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 1;
				
--Calculating revenue by Quarter
select qtr_id,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 1;
				
--Calculating revenue by country
select country,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 2 desc;
				
--Calculating revenue by city
select city,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 2 desc;
...
--To Convert orderdate from varchar to date function
select orderdate 
	from sales limit 5;
