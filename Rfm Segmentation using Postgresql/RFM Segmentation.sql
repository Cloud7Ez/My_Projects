...
--Calculating revenue and number of orders by productline		 
select productline,to_char(round(sum(sales), 0), '$999,999,999') revenue ,count(distinct ordernumber) no_of_orders
	from sales
		group by 1
			order by 3 desc;
			
--Calculating # of orders and revenue by year
select year_id,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue 
	from sales
		group by 1
			order by 2 desc;
			
--Calculating # of  orders and  revenue by month
select month_id,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 1;
				
--Calculating # of orders and revenue by Quarter
select qtr_id,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 1;
				
--Calculating revenue by country
select country,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 2 desc;
				
--Calculating revenue by city
select city,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue
		from sales
			group by 1
				order by 2 desc;
...
--Orders by Status
select status,count(*) number_of_orders,to_char(round(sum(sales), 0), '$999,999,999') revenue
	from sales
		group by 1
			order by 2 desc;

--Most Purchased Products by Each Customer
select customername,productcode,count(*) purchase_count,sum(sales) total_spent
	from sales
		group by 1,2
			order by 1, 3 desc;


--Calculating revenue by dealsize			
...
select month_id,TO_CHAR(SUM(sales), '$999,999,999') revenue,count(ordernumber) frequency
	from sales
...
select month_id,productline,TO_CHAR(SUM(sales), '$999,999,999') revenue,count(ordernumber) frequency
	from sales 
...
-- view all entries in rfm_segment
select * 
	from rfm_segment;
